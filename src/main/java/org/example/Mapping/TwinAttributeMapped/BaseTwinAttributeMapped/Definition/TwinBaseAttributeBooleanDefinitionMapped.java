package org.example.Mapping.TwinAttributeMapped.BaseTwinAttributeMapped.Definition;

import lombok.ToString;
import org.example.Mapping.Interfaces.TwinAttribute.BaseTwinAttribute.Definition.TwinBaseBooleanDefinition;
import org.example.Mapping.NewVersion.Abstract.MappedElementType;
import org.example.Util.LibraryNameSpaces;
import org.omg.sysml.lang.sysml.Classifier;

@MappedElementType(LibraryNameSpaces.TWIN_BOOLEAN)
@ToString(callSuper = true)
public class TwinBaseAttributeBooleanDefinitionMapped extends TwinBaseAttributeDefinitionMapped implements TwinBaseBooleanDefinition {

	public TwinBaseAttributeBooleanDefinitionMapped(Classifier sysmlElement) {
		super(sysmlElement);
	}
}
