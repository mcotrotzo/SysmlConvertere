...
public final class ReadManager {
	...
	private final SysMLInteractiveResult userContentResult;

	private final Element rootElement;
	private final LoadedResources loadedResources;

	public ReadManager(String userContentPath) {
		try {
			...
			Path userContent = requireDirectory(Path.of(userContentPath), "User content directory");
			...
			userContentResult = processDirectory(userContent, "User content directory");
			Resource userContentResource = requireResultResource(userContentResult, "User content directory");

			rootElement = Objects.requireNonNull(
					userContentResult.getRootElement(),
					"No root element was produced by the user content.");

			loadedResources = new LoadedResources(
					standardLibraryResources, dtLibraryResource, userContentResource, rootElement);
	...
	private static String readAllSysml(Path directory) throws IOException {
		...
		try (Stream<Path> paths = Files.walk(directory)) {
			files = paths.filter(Files::isRegularFile).filter(ReadManager::isSysmlFile)
		...
		try {
			return files.stream().map(ReadManager::readFile)
	...
	private static String readFile(Path file) {
		try {
			return "// SOURCE: " + file.toAbsolutePath().normalize() + System.lineSeparator()
	...
	private static void printIssues(SysMLInteractiveResult result, String description) {
		result.getIssues().forEach(issue -> System.err.printf("%s while processing %s at %d:%d: %s%n",
				issue.getSeverity(), description, issue.getLineNumber(), issue.getColumn(), issue.getMessage()));
	}
	...
	private static boolean isSysmlFile(Path path) {
		return path.getFileName().toString().toLowerCase(Locale.ROOT).endsWith(".sysml");
	}
	...
	public SysMLInteractiveResult getUserContentResult() {
